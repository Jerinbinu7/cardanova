...
export default function ContactForm({ onOpenQuoteModal }: ContactFormProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setContactItems([
          { icon: '✉️', label: 'Export Inquiry', value: parsed.emailSales || CONTACT_DETAILS[0].value },
          { icon: '💬', label: 'WhatsApp Trade Desk', value: parsed.whatsAppNumber || CONTACT_DETAILS[1].value },
          { icon: '📍', label: 'Registered Office', value: parsed.address || CONTACT_DETAILS[2].value },
          CONTACT_DETAILS[3],
        ]);
      }
    } catch (e) {
  ...
  return (
    <section id="contact" className="bg-[#FAF8F5] overflow-hidden"
          ...
          <div>
            ...
            <div className="space-y-4">
              {contactItems.map((item, i) => (
                ...
                >
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-[#112D15] border border-[#A18637]/30 text-lg">
                    {item.icon}
                  </div>
                  <div>
                    <span className="label-caps text-[#A18637]" style={{ fontSize: '0.55rem' }}>{item.label}</span>
          ...
          >
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full border border-[#C5A046]/50 bg-[#C5A046]/10 text-[#C5A046] text-3xl">
                    ✓
                  </div>
                ...
                >
                  ...
                  <div>
                    <label className="label-caps text-stone-400 mb-1.5 block" style={{ fontSize: '0.55rem' }}>Your Name *</label>
                    <input
                      type="text"
                      required
                      value={fullName}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="label-caps text-stone-400 mb-1.5 block" style={{ fontSize: '0.55rem' }}>Email *</label>
                      <input
                        type="email"
                        required
                        value={email}
                    ...
                    <div>
                      <label className="label-caps text-stone-400 mb-1.5 block" style={{ fontSize: '0.55rem' }}>Country / Port *</label>
                      <input
                        type="text"
                        required
                        value={country}
                  ...
                  <div>
                    <label className="label-caps text-stone-400 mb-1.5 block" style={{ fontSize: '0.55rem' }}>Message / Volume Requirement</label>
                    <textarea
                      rows={3}
                      value={message}
